...
def parse_html(html):
    ...
    # print data
    while True:
        tmp, data = common.match(data, 'node-type="article-content', 'data-id')
        if tmp == '':
            break
        ...
        title, data = common.match(data, 'node-type="article-title" alt="', '"')
        print title

        uploadTime, data = common.match(data, '<span class="stat-time">', '</span>')
        print uploadTime
        uploadTime = uploadTime[: len('2017-03-25')]
        print uploadTime

        playCount, data = common.match(data, '<span class="stat-eye-num">', '</span>')
        print playCount
